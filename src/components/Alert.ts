...
export function Alert(props: AlertProps): NixTemplate {
    ...
    const instanceId = _alertId++;
    const contentId = `nix-alert-content-${instanceId}`;
    const titleId = `nix-alert-title-${instanceId}`;
    ...
    return transition(
        () =>
            visible.value
                ? html`
                    <div class=${classes} role="alert" aria-labelledby=${title ? titleId : undefined} aria-describedby=${contentId}>
                        ${icon
                        ? html`
                                <svg class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" aria-hidden="true">
                        ...
                        <div class="flex-1 min-w-0">
                            ${title
                        ? html`<h4 id=${titleId} class="text-sm font-semibold mb-1">${title}</h4>`
                        : ""}
                            <div id=${contentId} class="text-sm opacity-90">${children}</div>
                        </div>
                        ${dismissible
                        ? html`
                                <button
